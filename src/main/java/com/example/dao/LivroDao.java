...
public class LivroDao {
    ...
    public Livro consultarPorId(int id) { // Alterado para consultar por ID
    ...
    public void alterar(Livro livro) {
        ...
        try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
            ...
            stmt.setInt(4, livro.getAno());
            stmt.setBoolean(5, livro.isDisponivel());
            stmt.setInt(6, livro.getId()); // Alterado para usar ID
            stmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    //marcar um livro como emprestado ou devolvido
